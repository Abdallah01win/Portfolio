...
<template>
  <section id="projects"
    class="3xs:max-w-[320px] 2xs:max-w-[360px] xs:max-w-[390px] sm:max-w-[475px] md:max-w-[640px] lg:max-w-[768px] xl:max-w-[1024px] 2xl:max-w-[1280px] mx-auto relative py-14 sm:py-18 md:py-24 border-b border-myGray-200/40">
    <div class="mb-10">
      <div
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-14 sm:gap-16 lg:gap-12">
      <div v-for="(item, index) in projects" :key="item.title"
        ...
        @mouseleave="showDiv[index] = false" @click.self="showPreview(index)">
        <div v-show="showDiv[index]" class="h-full bg-myGray-200/30 flex items-center justify-center transition-opacity">
