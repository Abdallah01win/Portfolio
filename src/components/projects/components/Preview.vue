...
<template>
  <div v-if="isPreviewShown" @keydown.escape="closePreview"
    class="fixed top-0 left-0 w-full h-screen z-10 bg-myGray-200/80 glass" tabindex="0">
    <div
    ...
    <Splide :options="options" aria-label="projectImages">
      <SplideSlide v-for="(image, index) in project.images" :key="index">
        <div class="xl:max-w-[1024px] 2xl:max-w-[1280px] mx-auto flex items-center h-screen">
          <img :src="image" :alt="project?.title" class="w-full h-[50%] sm:h-[60%] md:h-[65%] lg:h-full" />
        </div>
      </SplideSlide>
    </Splide>
  </div>
