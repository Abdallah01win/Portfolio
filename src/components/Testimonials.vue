...
<template>
  <section id="testimonials"
    ...
    <div class="border border-myGray-200/40 rounded-3xl">
      <Splide :options="options" aria-label="testimonials">
        <SplideSlide v-for="(item, index) in testimonials" :key="index">
          <div class="flex flex-col items-center px-8 md:px-12 py-8 md:py-10 lg:py-16">
            <div>
              <Quote class="w-6 md:w-8 h-6 md:h-8 text-myGray-200/40 mb-4 md:mb-9" />
            </div>
            <div class="sm:text-xl md:text-3xl font-primaryBold mb-3 md:mb-5 lg:mb-3">
              "{{ item.title }}"
            </div>
            <p
            ...
            <div class="inline-flex items-center">
              ...
              <span class="flex-grow flex flex-col pl-4 gap-y-0.5">
                ...
                <span class="text-sm text-myGray-100">{{ item.role }}</span>
              </span>
            </div>
          </div>
        </SplideSlide>
